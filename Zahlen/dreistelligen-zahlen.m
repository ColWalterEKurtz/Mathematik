clear all;

printf("\\begin{exercise}\n");
printf("      {ID-38b1d2079990f8cb9959bc098be8c51c31048ee1}\n");
printf("      {Dreistelligen Zahlen}\n");
printf("  \\ifproblem\\problem\n");
printf("    Bei dreistelligen Zahlen kann man viele Besonderheiten untersuchen.\n");
printf("    \\begin{enumerate}[a)]\n");
printf("      \\item So gibt es dreistellige Zahlen, deren letzte Ziffer (die Einerziffer)\n");
printf("            die Summe der ersten beiden Ziffern ist. Wie viele Zahlen dieser Art\n");
printf("            gibt es?\n");
printf("      \\item Wie viele dreistellige Zahlen gibt es, bei denen die Einerziffer das\n");
printf("            Produkt der ersten beiden Ziffern ist?\n");
printf("      \\item Gib alle dreistelligen Zahlen an, bei denen sich die Einerziffer ergibt,\n");
printf("            wenn man die Hunderterziffer durch die Zehnerziffer teilt.\n");
printf("    \\end{enumerate}\n");
printf("  \\fi\n");
printf("  %%\\ifoutline\\outline\n");
printf("  %%\\fi\n");
printf("  %%\\ifoutcome\\outcome\n");
printf("  %%\\fi\n");
printf("\\end{exercise}\n");

