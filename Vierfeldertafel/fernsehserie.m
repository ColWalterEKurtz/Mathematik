clear all;

printf("\\begin{exercise}\n");
printf("      {ID-f80f0592abc80f392a5b221b3308144861f85874}\n");
printf("      {Fernsehserie}\n");
printf("  \\ifproblem\\problem\\par\n");
printf("    Eine Fernsehanstalt möchte die neue amerikanische Serie \\glqq Lewis And Clark\\grqq{}\n");
printf("    übernehmen. Sie befragt daher im Anschluss an eine Pilotsendung die Zuschauer:\n");
printf("    Von den Zuschauern, die diese Sendung gesehen hatten, waren \\pc{55} älter als\n");
printf("    30 Jahre. \\pc{30} von diesen und \\pc{60} der übrigen fanden die Sendung gut.\n");
printf("    \\begin{enumerate}\n");
printf("      \\item Berechne den Anteil der Zuschauer, die eine positive Meinung\n");
printf("            von der Sendung hatten.\n");
printf("      \\item Ein Zuschauer von \\glqq Lewis And Clark\\grqq, der sich positiv\n");
printf("            darüber geäußert hat, wird zufällig ausgewählt. Mit welcher\n");
printf("            Wahrscheinlichkeit ist er älter als 30 Jahre?\n");
printf("    \\end{enumerate}\n");
printf("  \\fi\n");
printf("  %%\\ifoutline\\outline\\par\n");
printf("  %%\\fi\n");
printf("  %%\\ifoutcome\\outcome\\par\n");
printf("  %%\\fi\n");
printf("\\end{exercise}\n");

